...
test('a token issued by the csrfToken() helper validates through the web pipeline (no special arg)', function () {
    // Swap csrf for an in-memory manager so the helper + middleware share it (no native session in CLI).
    $storage = new class () implements \Symfony\Component\Security\Csrf\TokenStorage\TokenStorageInterface {
        /** @var array<string, string> */
        private array $t = [];
    ...
    Kernel::app()->instance('csrf', new \Symfony\Component\Security\Csrf\CsrfTokenManager(
        new \Symfony\Component\Security\Csrf\TokenGenerator\UriSafeTokenGenerator(),
        $storage
    ));

    // Issue via the helper with NO explicit form name — default must be 'web' to match the middleware.
    $token = csrfToken(); // defaults to 'web', resolves the shared container manager
