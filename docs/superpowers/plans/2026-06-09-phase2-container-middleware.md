...
> If 2.5 produced churn, STOP and split this into its own plan. Otherwise port these one task each, each with a test, each preserving BC via the existing helper/shim:

- [ ] **ConfigProvider:** move `captureConfig()` into a provider (or keep `captureConfig()` but expose `config` as a container binding). Low value vs risk — keep `config()` helper unchanged; just register the `Config` object as `config` in the container. Test: `Kernel::app()->get('config')` is the same `Config` as `config()`.
- [ ] **ViewProvider:** register a Twig/Smarty factory in the container; `BaseController` resolves it instead of `new`-ing in `_loadInit`. Preserve locale/`tJson` globals behavior. Test: container resolves a configured Twig environment. (Coordinate with the Smarty-5 tracked debt.)
- [ ] **MailProvider:** bind a `mailer` service wrapping the Symfony Mailer DSN currently built ad-hoc in `helpers.php::newMailerDsn()`; keep the helper as a shim delegating to the binding. Test: `mailer` resolves.
- [ ] **AuthProvider:** bind `jwt` (the `Ions\Security\Jwt` built from env/config) and `auth` (Sentinel guard wrapper); `AuthMiddleware` resolves `jwt` from here instead of building it inline. Test: `jwt` resolves and round-trips a token. (Sets up Phase 5's pluggable `UserProvider`.)
- [ ] **RoutingProvider:** bind the `RouteCollection`/router building so `handle()` resolves it from the container rather than static `Kernel::RouteCollection()`. Test: `router`/`routes` resolves. (Optional; only if it reduces static coupling cleanly.)
