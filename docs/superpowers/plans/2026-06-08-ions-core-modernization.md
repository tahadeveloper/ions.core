...
> **✅ STATUS — COMPLETE (3.0.0, 2026-06-10).** All phases shipped: **0** (foundation/tooling/CI), **1** (security hardening), **2** (DI container + providers + middleware pipeline), **3** (HTTP/routing/controllers + ExceptionHandler + Twig-only), **4 incl. 4.6** (query-injection fix, Illuminate 9→**11**, Symfony 6→**7**, Monolog 2→**3**, Pest 2→**3**, RedBean/Smarty/verot removed, MySQL CI), **5** (pluggable auth: UserProvider, JWT refresh/revocation/leeway, rate-limit, CSRF enforcement, Guard deprecated), **6** (level-8 core PHPStan + strict_types, generators, README/`/docs`/CHANGELOG, `IonDisk::download` fix). 1 trivial test → **200 tests** green; PHPStan + CS-Fixer clean; CI on PHP 8.2/8.3 + MySQL. See `CHANGELOG.md` + `UPGRADE-3.0.md`.
...
8. **Tracked from Phase 2.6 (deferred providers):** (a) **ViewProvider** — register a container-bound, locale-aware Twig/Smarty view factory and have `BaseController::_loadInit` resolve it instead of `new`-ing per request; design alongside the Smarty-5 decision (item 6). (b) **RoutingProvider** (optional) — bind `router`/`routes` so `handle()` resolves the `RouteCollection` from the container instead of static `Kernel::RouteCollection()`, if it reduces static coupling cleanly during the routing consolidation (item 1). (Phase 2 completed AuthProvider/ConfigProvider/MailProvider; note Phase 2 already delivered `ControllerDispatcher` + container-resolved controllers, partially satisfying item 2.)
...
5. **Database:** Eloquent 9→11 deltas; RedBeanPHP removed — the `'redbean'` value in `app.database_engine` is now ignored (a deprecation warning is logged). Use the `'db'` (Eloquent) engine; port any RedBean (`R::`) usage to Eloquent.
   - **RedBeanPHP removed:** the `'redbean'` value in `app.database_engine` is now ignored (logged). Use the `'db'` (Eloquent) engine; port any RedBean (`R::`) usage to Eloquent.
