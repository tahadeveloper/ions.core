...
class QueryBuilderRequest extends Request
{
    ...
    public function includes(): Collection
    {
        ...
        if (! is_array($includeParts) && $includeParts !== null) {
    ...
    {
        $limitsParameterName = config('query-builder.parameters.limit');

        $limitsPerTable = collect($this->getRequestData($limitsParameterName));

        if ($limitsPerTable->isEmpty()) {
            return collect();
        }

        return $limitsPerTable->map(function ($fields) {
            return $fields;
        });
    }
