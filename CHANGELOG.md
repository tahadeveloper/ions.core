...
- **Zero-config provider auto-discovery** — when `app.providers` is not set, `Ions\Foundation\Discovery::providers()` registers the framework defaults plus providers discovered from the host `{src|app}/Providers/` directory (single glob per boot, `src/` → `app/` fallback preserved) and from installed composer packages declaring `extra.ions.providers` (read once per process from `vendor/composer/installed.json`, memoized). Host providers run last so they can override package/framework bindings. Escape hatches: an explicit `app.providers` list bypasses discovery entirely (BC); `app.discovery => false` keeps pure defaults. Test seam `Discovery::useMetadata()` / `Discovery::reset()` (wired into `Kernel::resetForTesting()`). See [docs/packages.md](docs/packages.md) and [docs/config.md](docs/config.md#appproviders).
